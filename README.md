...
### Web search — ground once, fan out analysis (orchestrator-decided)

Code workers stay ungrounded on purpose (a deterministic, check-verified task must not be silently steered by whatever a search returned). But **research** is worthless on stale model priors — so research gets web grounding, and the **default** pattern is designed to be cheap and hallucination-proof:

**DEFAULT — ground once, fan out** (`templates/grounded-research/`). The orchestrator runs a small set of web searches **itself** (with its own search tool, off any rate-limited worker-search tier), vets the sources, and writes ONE cited `corpus.md`. Then cheap analysis workers (`engine: opencode`, no web) fan out over that same corpus — each reads it as source material and cites its URLs. The kit's check (`checks/grounded-analysis.py`) **cross-references every cited URL against the corpus**, so a worker can't smuggle in a hallucinated source. One agentic worker re-searches on *every* step, so N augmented workers can fire dozens of searches; grounding once collapses that to a handful, on a backend that can't self-exhaust.

**FALLBACK — worker-augment** (this fleet: Feeder's free Ollama `web_search`). When research is too broad to pre-gather, workers search live via the `opencode-research` engine (`engines/opencode-research.sh`), which flips the OpenCode provider headers to `X-Consumer: ringer-research` + `X-Augment: force` per run (Feeder augments the request and tags it). Because the free tier is rate-capped, run this at **low `max_parallel` (2–3)** and treat it as unverified until audited:

- **Pre-flight:** `scripts/search_preflight.py` probes whether augmentation is live (exit 0) or throttled (exit 2) before you spend a swarm.
- **Post-hoc audit (mandatory):** Feeder logs an `augmented` flag + an `augment_skipped` reason (`throttled | no-results | no-config | error`) per call. After the run, any research call that requested augment but shows `augmented=false` never got fresh grounding — **do not ship it**; re-ground yourself. Back off specifically on `augment_skipped='throttled'`.

Either way the orchestrator owns the choice per task (auditable in the manifest, no `ringer.py` change), and it starts with a **swarm-worthiness triage** — a brief that's really one question is answered directly (grounded), not fanned out at all. A generic (non-Feeder) clone wires the same shape with any web-search MCP: orchestrator grounds → shared corpus → analysis workers → cross-ref check.
